package com.otienochris.springmvcguru.services.mapservices;

import com.otienochris.springmvcguru.models.DomainObject;

import java.util.*;

public abstract class AbstractMapService {

    protected Map<Integer, DomainObject> domainMap; // this will act as our database

    public AbstractMapService() {
        domainMap = new HashMap<>(); // instantiate the db
//        loadDomainObject(); // load the data
    }

    public List<DomainObject> listAll(){ // return list from the values from the map
        return new ArrayList<>(domainMap.values());
    }

    public DomainObject getById(Integer id){
        return domainMap.get(id);
    }

    public DomainObject saveOrUpdate(DomainObject domainObject){
        if (domainObject != null){
            if (domainObject.getId() == null){
                domainObject.setId(getNextKey());
            }
            domainMap.put(domainObject.getId(), domainObject);

            return domainObject;
        } else {
            throw new RuntimeException("Object can't be null.");
        }
    }

    public void delete(Integer id){ domainMap.remove(id); }

    private Integer getNextKey(){return Collections.max(domainMap.keySet()) + 1; }

//    protected abstract void loadDomainObject();
}
